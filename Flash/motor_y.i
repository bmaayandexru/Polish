...
#line 11 "Main\\Motor_Y.c"
#line 12 "Main\\Motor_Y.c"
#line 1 "Main\\msg.h"
...
#line 14 "Main\\Motor_Y.c"

TIMER_CONTROL_STRUCT tcs_MotorY; 	
TFMC 	mc_Y;
TFMS 	ms_Y;
		...
		case 0: 
			...
			if (QGetPin(28)) {
				...
				*(mc_Y.FrReq)=*(mc_Y.FrMax);
				...
				if (*(ms_Y.FrCur) < (*(mc_Y.FrReq)/2)) {
					
					*(ms_Y.FrCur) += *(mc_Y.Accel) * (ms_Y.Tcur);
					if (*(ms_Y.FrCur) > (*(mc_Y.FrReq)/2)) *(ms_Y.FrCur) = *(mc_Y.FrReq)/2;
					ms_Y.Tcur = ((u64)625000000000000) / (*(ms_Y.FrCur));
					ms_Y.LPOFF = ( ms_Y . Tcur > (ms_Y . LPON+1+0) ? (ms_Y . Tcur-ms_Y . LPON-0) : 1);		
				} 
		...
		case 1: 
			if (*(ms_Y.FrCur) <= *(mc_Y.FrMin)) {
				...
				*ms_Y.StagePos = 2;
				...
				if (*(ms_Y.FrCur) > *(mc_Y.Accel) * 4 * (ms_Y.Tcur))
					*(ms_Y.FrCur) -= *(mc_Y.Accel) * 4 * (ms_Y.Tcur);
				else *(ms_Y.FrCur) = 0;
...
__inline void MovePosY(void){
	...
	if (*(mc_Y.ReqPos) < vY_MIN_POS) *(mc_Y.ReqPos)=vY_MIN_POS;
	if (*(mc_Y.ReqPos) > vY_MAX_POS) *(mc_Y.ReqPos)=vY_MAX_POS;
	...
	if (*(mc_Y.MovePos) == 1) {
		    ...
		    ((*(mc_Y.ReqPos) > *(ms_Y.Pos))&&(*(ms_Y.Dir)==-1)))
		{
			...
			if (*(ms_Y.FrCur) > *(mc_Y.FrMin)) {
				...
				if (*(ms_Y.FrCur) > *(mc_Y.Accel) * 2 *(ms_Y.Tcur))
					*(ms_Y.FrCur) -= *(mc_Y.Accel) * 2 * (ms_Y.Tcur);
				else *(ms_Y.FrCur) = 0;
				if (*(ms_Y.FrCur) < *(mc_Y.FrMin)) {
					*(ms_Y.FrCur) = *(mc_Y.FrMin);
					ms_Y.Lacc = 0;
				}	
				if (ms_Y.Lacc>1) ms_Y.Lacc-=2;
				ms_Y.Tcur = ((u64)625000000000000) / (*(ms_Y.FrCur));
				ms_Y.LPOFF = ( ms_Y . Tcur > (ms_Y . LPON+1+0) ? (ms_Y . Tcur-ms_Y . LPON-0) : 1);		
			} else {
...
__inline void MoveBtnY(void){
	if 	(((!*(mc_Y.MoveBtn))&&(*(ms_Y.FrCur) <= *(mc_Y.FrMin))) || 
		(*(ms_Y.Pos) <= vY_MIN_POS)&&(*(mc_Y.ReqDir) == -1) ||
		(*(ms_Y.Pos) >= vY_MAX_POS)&&(*(mc_Y.ReqDir) == 1)) 

	{
		
		
		ms_Y.Tcur = ((u64)625000000000000) / *(mc_Y.FrMin);
		...
		if (!*(mc_Y.MoveBtn)) {
			...
			if (*(ms_Y.Dir)!=*(mc_Y.ReqDir)) {
				...
				if 	( 
						(((*(ms_Y.Pos)-(ms_Y.Lacc+10)) < vY_MIN_POS)&&(*(mc_Y.ReqDir) == -1)) ||
						(((*(ms_Y.Pos)+(ms_Y.Lacc+10)) > vY_MAX_POS)&&(*(mc_Y.ReqDir) == 1)) 
					)
				{
...
void	MotorY_Handler(void){
	{if (ms_Y . LPOFF < 1) ms_Y . LPOFF=1;};	
	if (ms_Y.Level==0) {
		if (fmvc.Enable==0)  { 
			
			if (*(ms_Y.Dir) == 0) {
				Msg("YD0\r\n");
				if (QGetPin(4)) *(ms_Y.Dir) = 1;
				else *(ms_Y.Dir)=-1;
			}
			ClrSY = 0;
			...
			if (ms_Y.Level) {
				...
				QClrPin(3);	
				QClrPin(6);	
				tcs_MotorY . interval = ms_Y . LPOFF;		
			} else {
				QClrPin(3);	
				QClrPin(6);	
				tcs_MotorY . interval = ms_Y . LPOFF;		
