...
#line 11 "Main\\Motor_Z.c"
#line 12 "Main\\Motor_Z.c"
#line 1 "Main\\msg.h"
...
#line 14 "Main\\Motor_Z.c"

TIMER_CONTROL_STRUCT tcs_MotorZ; 	
TFMC 	mc_Z;
TFMS 	ms_Z;
				...
				*(mc_Z.FrReq)=*(mc_Z.FrMax);
				...
				if (*(ms_Z.FrCur) < *(mc_Z.FrReq)) {
					...
					*(ms_Z.FrCur) += *(mc_Z.Accel) * (ms_Z.Tcur);
					...
					ms_Z.Tcur = ((u64)625000000000000) / (*(ms_Z.FrCur));
					ms_Z.LPOFF = ( (ms_Z . Tcur) > (ms_Z . LPON+1+0) ? ((ms_Z . Tcur)-ms_Z . LPON-0) : 1);		
				} 
		...
		case 1: 
			if (*(ms_Z.FrCur) <= *(mc_Z.FrMin)) {
				...
				*ms_Z.StagePos = 2;
				...
				if (*(ms_Z.FrCur) > *(mc_Z.Accel) * (ms_Z.Tcur))
					*(ms_Z.FrCur) -= *(mc_Z.Accel) * (ms_Z.Tcur);
				else *(ms_Z.FrCur) = 0; 
...
__inline void MovePosZ(void){
	...
	if (*(mc_Z.ReqPos) < vZ_MIN_POS) *(mc_Z.ReqPos)=vZ_MIN_POS;
	if (*(mc_Z.ReqPos) > vZ_MAX_POS) *(mc_Z.ReqPos)=vZ_MAX_POS;
...
__inline void MoveBtnZ(void){
	...
	if 	(((!*(mc_Z.MoveBtn))&&(*(ms_Z.FrCur) <= *(mc_Z.FrMin))) || 
		(*(ms_Z.Pos) <= vZ_MIN_POS)&&(*(mc_Z.ReqDir) == -1) ||
		(*(ms_Z.Pos) >= vZ_MAX_POS)&&(*(mc_Z.ReqDir) == 1)) 

	{
		
		
		ms_Z.Tcur = ((u64)625000000000000) / *(mc_Z.FrMin);
		...
		if (!*(mc_Z.MoveBtn)) {
			...
			if (*(ms_Z.Dir)!=*(mc_Z.ReqDir)) {
				...
				if 	( 
						(((*(ms_Z.Pos)-(ms_Z.Lacc+10)) < vZ_MIN_POS)&&(*(mc_Z.ReqDir) == -1)) ||
						(((*(ms_Z.Pos)+(ms_Z.Lacc+10)) > vZ_MAX_POS)&&(*(mc_Z.ReqDir) == 1)) 
					)
				{
...
void	MotorZ_Handler(void){
	{if (ms_Z . LPOFF < 1) ms_Z . LPOFF=1;};
	if (ms_Z.Level==0) {
		if (fmvc.Enable==0)  { 
			
			if (*(ms_Z.Dir) == 0) {
				Msg("ZD0\r\n");
				if (QGetPin(10)) *(ms_Z.Dir) = 1;
				else *(ms_Z.Dir)=-1;
			}
			ClrSZ = 0;
			...
			if (ms_Z.Level) {
				...
				QClrPin(9);	
				tcs_MotorZ . interval = ms_Z . LPOFF;		
			} else {
				QClrPin(9);	
				tcs_MotorZ . interval = ms_Z . LPOFF;		
			}	
