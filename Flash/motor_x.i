...
#line 11 "Main\\Motor_X.c"
#line 12 "Main\\Motor_X.c"
#line 1 "Main\\msg.h"
...
#line 14 "Main\\Motor_X.c"

TIMER_CONTROL_STRUCT tcs_MotorX; 	
TFMC 	mc_X;
TFMS 	ms_X;
		...
		case 0: 
			...
			if (QGetPin(30)) {
				...
				*(mc_X.FrReq)=*(mc_X.FrMax);
				...
				if (*(ms_X.FrCur) < (*(mc_X.FrReq)/2)) {
					
					*(ms_X.FrCur) += *(mc_X.Accel) * (ms_X.Tcur);
					if (*(ms_X.FrCur) > (*(mc_X.FrReq)/2)) *(ms_X.FrCur) = *(mc_X.FrReq)/2;
					ms_X.Tcur = ((u64)625000000000000) / (*(ms_X.FrCur));
					ms_X.LPOFF = ( ms_X . Tcur > (ms_X . LPON+1+0) ? (ms_X . Tcur-ms_X . LPON-0) : 1);		
				} 
		...
		case 1: 
			if (*(ms_X.FrCur) <= *(mc_X.FrMin)) {
				...
				*ms_X.StagePos = 2;
				...
				if (*(ms_X.FrCur) > *(mc_X.Accel) * 4 * (ms_X.Tcur))
					*(ms_X.FrCur) -= *(mc_X.Accel) * 4 * (ms_X.Tcur);
				else *(ms_X.FrCur) = 0; 
...
__inline void MovePosX(void){
	...
	if (*(mc_X.ReqPos) < vX_MIN_POS) *(mc_X.ReqPos)=vX_MIN_POS;
	if (*(mc_X.ReqPos) > vX_MAX_POS) *(mc_X.ReqPos)=vX_MAX_POS;
	if (*(mc_X.MovePos) == 1) {
		    ...
		    ((*(mc_X.ReqPos) > *(ms_X.Pos))&&(*(ms_X.Dir)==-1)))
		{
			...
			if (*(ms_X.FrCur) > *(mc_X.FrMin)) {
				...
				if (*(ms_X.FrCur) > *(mc_X.Accel) * 2 * (ms_X.Tcur))
					*(ms_X.FrCur) -= *(mc_X.Accel) * 2 * (ms_X.Tcur);
				else *(ms_X.FrCur) = 0; 
				if (*(ms_X.FrCur) < *(mc_X.FrMin)) {
					*(ms_X.FrCur) = *(mc_X.FrMin);
					ms_X.Lacc = 0;
				}
				if (ms_X.Lacc>1) ms_X.Lacc-=2;
...
__inline void MoveBtnX(void){
	...
	if 	(((!*(mc_X.MoveBtn))&&(*(ms_X.FrCur) <= *(mc_X.FrMin))) || 
		(*(ms_X.Pos) <= vX_MIN_POS)&&(*(mc_X.ReqDir) == -1) ||
		(*(ms_X.Pos) >= vX_MAX_POS)&&(*(mc_X.ReqDir) == 1)) 

	{
		
		
		ms_X.Tcur = ((u64)625000000000000) / *(mc_X.FrMin);
		...
		if (!*(mc_X.MoveBtn)) {
			...
			if (*(ms_X.Dir)!=*(mc_X.ReqDir)) {
				...
				if 	( 
						(((*(ms_X.Pos)-(ms_X.Lacc+10)) < vX_MIN_POS)&&(*(mc_X.ReqDir) == -1)) ||
						(((*(ms_X.Pos)+(ms_X.Lacc+10)) > vX_MAX_POS)&&(*(mc_X.ReqDir) == 1)) 
					)
				{
...
void	MotorX_Handler(void){
	{if (ms_X . LPOFF < 1) ms_X . LPOFF=1;};
	if (ms_X.Level == 0) {
		if (fmvc.Enable == 0)  { 
			
			if (*(ms_X.Dir) == 0) {
				Msg("XD0\r\n");
				if (QGetPin(1)) *(ms_X.Dir) = 1;
				else *(ms_X.Dir)=-1;
			}
			...
			if (ms_X.Level) {
				...
				QClrPin(0);	
				tcs_MotorX . interval = ms_X . LPOFF;		
			} else {
				QClrPin(0);	
				tcs_MotorX . interval = ms_X . LPOFF;		
			}	
