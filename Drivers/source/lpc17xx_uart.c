...
//******************** Наши функции детализированы под быструю работу и персонализацию UARTов
// UART0_Send - передает 1 байт по неблокирующему соединению UATR0
uint32_t UART0_Send(uint8_t Data)
{
		if (!(LPC_UART0->LSR & UART_LSR_THRE)) return 0; // ничего не вышло
		LPC_UART0->THR = Data & UART_THR_MASKBIT;		// тут передаем
		return 1; // все ок
}
// UART1_Send - передает 1 байт по неблокирующему соединению UATR1
uint32_t UART1_Send(uint8_t Data)
{
		if (!(LPC_UART1->LSR & UART_LSR_THRE)) return 0; // ничего не вышло
		LPC_UART1->THR = Data & UART_THR_MASKBIT;		// тут передаем
		return 1; // все ок
}
//********************
/*********************************************************************//**
...
//	uint8_t badChar;
	...
	if (flag == BLOCKING) {
		...
		while (bToRecv){
			timeOut = UART_BLOCKING_TIMEOUT;
			// ждем 0-й бит в регистре LSR если от 1 то FIFO UARTa не пуста
			while (!(UARTx->LSR & UART_LSR_RDR)){
				if (timeOut == 0) break;
				timeOut--;
			}
			// Time out!
			if(timeOut == 0) break; // не дождались выходим
			// Get data from the buffer
			(*pChar++) = UART_ReceiveByte(UARTx); // читаем регистр RBR
			bToRecv--;
			bRecv++;
		}
	}
	// None blocking mode
		...
		while (bToRecv) {
			if (!(UARTx->LSR & UART_LSR_RDR)) {
				...
				// моя проверка паритета
//				if (!(UARTx->LSR & UART_LSR_PE)) {
					// нет ошибки
...
//				} 
				// ошибка паритета
//				else badChar = UART_ReceiveByte(UARTx); 
			}
		}
	}
	return bRecv;
